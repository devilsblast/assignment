...
const User = ({children}) => {
    ...
    const getUsers = function (page,limit){
        let array=[];
            for(let i=(page-1)* limit; i< Math.min((page* limit),userdata.length); i++){
           ...
           const getsingledelete = function (id,page,limit){
            
             setUserdata(userdata.filter((user)=>user.id !== id));
             return getUsers(page,limit);
          }

       const delchecked = function (page,limit){
        let array=[];
        for(let i=0;i<userdata.length;i++){
            if(userdata[i].ischecked) continue;
            else{
                array.push(userdata[i]);
            }
        }
        ...
        setUserdata(array);
          ...
          const getsearch = function (input,page,limit){
            
            setTemp(userdata.filter((user) => user.name.toLowerCase().includes(input.toLowerCase())));
            let array=[];
            for(let i=(page-1)* limit; i< Math.min((page* limit),temp.length); i++){
                array.push(temp[i]);
            }
            return array; 
        }

        const addcheck = function(id,checked){
            let temp=userdata.map( user => user.id === id ? {...user, ischecked : checked} : user);
        setUserdata(temp);
        }
        const addcheckall = function (page,limit,checked){
            for(let i=(page-1)* limit; i< Math.min((page* limit),userdata.length); i++){
                userdata[i]={...userdata[i],ischecked:checked};
            }
        }

        const updatedmain = function (editedUser){
